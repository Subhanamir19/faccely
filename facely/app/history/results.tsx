...
import type { ScanHistoryItem } from "@/lib/api/history";
import { fetchHistoryPhotoArchive } from "@/lib/historyArchive";
  ...
  return (
    <Animated.View entering={FadeIn.delay(200)} style={emptyStyles.container}>
      ...
      <Text style={emptyStyles.sub}>
        Run your first scan to see your results here
      </Text>
    </Animated.View>
  );
}
  ...
  return (
    <View style={headerStyles.wrap}>
      ...
      <View style={headerStyles.titleRow}>
        <View style={{ flex: 1 }}>
          <Text style={headerStyles.title}>Scan Results</Text>
...
export default function HistoryScreen() {
  ...
  const load = useCallback(async () => {
    setError(null);
    try {
      const data = await fetchHistoryPhotoArchive(30);
      setScans(
        data.map((item) => ({
          id: item.id,
