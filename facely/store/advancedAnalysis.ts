...
export const useAdvancedAnalysis = create<State & Actions>((set, get) => ({
  ...
  fetch: async () => {
    ...
    console.log("[advancedAnalysis] fetch() called — scanId:", scanId, "hasImageUri:", !!imageUri, "hasScores:", !!scores);

    if (!scores || !imageUri) {
      console.warn("[advancedAnalysis] blocked — missing scores or imageUri");
    ...
    console.log("[advancedAnalysis] cache check — loading:", loading, "cachedScanId:", cachedScanId, "scanId:", scanId, "hasData:", !!data);
    if (loading) { console.log("[advancedAnalysis] skipped — already loading"); return; }
    if (data && cachedScanId === scanId && scanId !== null) {
      console.log("[advancedAnalysis] skipped — cache hit (scanId matches)");
      return;
    }

    set({ loading: true, error: null });
    try {
      console.log("[advancedAnalysis] calling /analyze/advanced-explain...");
      const result = await fetchAdvancedAnalysis(imageUri, scores, scanId);
      console.log("[advancedAnalysis] fetch OK — groups returned:", Object.keys(result));
      ...
      console.log("[advancedAnalysis] fetch complete — invalidating insights and reloading");
      ...
      console.error("[advancedAnalysis] fetch FAILED:", message);
