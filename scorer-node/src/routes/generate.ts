...
// POST /generate/ten-by-ten — AI face enhancement using gpt-image-2.
...
router.post("/ten-by-ten", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    const response = await _openai.images.edit({
      ...
      n: 1,
      size: "1024x1024",
    } as any); // cast: response_format not needed for GPT image edits here
...
/* -------------------------------------------------------------------------- */
/*   POST /potential-face-dev                                                 */
/* -------------------------------------------------------------------------- */

router.post("/potential-face-dev", upload.single("image"), async (req, res) => {
  const t0 = Date.now();
  const userId = res.locals.userId;
  const file = req.file;
  ...
  if (!file) {
    return res.status(400).json({
      error: "missing_image",
      hint: "Include the photo as an 'image' field in multipart/form-data.",
    });
  }
  ...
  try {
    ...
    const b64 = response.data?.[0]?.b64_json;
    if (!b64) {
      console.error("[/generate/potential-face-dev] OpenAI returned no b64_json", response);
      return res.status(502).json({ error: "no_image_returned" });
    }
    ...
    console.log("[/generate/potential-face-dev] ms =", Date.now() - t0);
  }
});
